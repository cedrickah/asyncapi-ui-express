 ...
 * ```
 */
export function serve(outputUrl: string): RequestHandler {
    if (!existsSync(outputUrl) || !statSync(outputUrl).isDirectory()) {
        throw new Error(
            `Provided outputUrl "${outputUrl}" is not a valid directory`
        );
    }
